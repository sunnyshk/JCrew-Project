...
export const Salegirl = () => {
  const [salegirl, setgirl] = useState([]);

  useEffect(() => {
    async function getData() {
      const data = await fetch("https://ancient-dawn-26797.herokuapp.com/sale_girl?_limit=12").then((d) =>
        d.json()
      );
      setgirl(data);
    }
  ...
  return (
    <div className="for">
      ...
      <div id="div-6">
        {salegirl.map((e) => {
          return (
            <Link to={`/Salegirl/${e.id}/sale_girl`} id="card-1">
              <div id="img-1">
...
// {salegirl.map(e =>
// <Link to={`/Salegirl/${e.id}/sale_girl`} className="mybox">
