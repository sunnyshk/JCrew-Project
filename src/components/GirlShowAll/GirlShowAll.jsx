...
export const Girlshopall = () => {
  const [girlshort, setshout] = useState([]);

  useEffect(() => {
    async function getData() {
      const data = await fetch("https://ancient-dawn-26797.herokuapp.com/girlshop_all?_limit=12").then((d) =>
        d.json()
      );
      setshout(data);
    }
  ...
  return (
    <div className="for">
      ...
      <div id="div-6">
        {girlshort.map((e) => { 
          return (  
            <Link to={`/Girlshopall/${e.id}/girlshop_all`} id="card-1">
              <div id="img-1">
