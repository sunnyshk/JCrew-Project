...
export const Girlshorts = () => {
  const [girlshort, setshout] = useState([]);

  useEffect(() => {
    async function getData() {
      const data = await fetch("https://ancient-dawn-26797.herokuapp.com/girlsshorts?_limit=12").then((d) =>
        d.json()
      );
      setshout(data);
    }
  ...
  return (
    <div className="for">
      ...
      <div id="top2">
        <h1 id="top-text2">Girl's Shorts</h1>
      </div>
      <div id="top3">
        <select name="" id="">
      ...
      <div id="div-6">
        {girlshort.map((e) => {
          return (
            <Link to={`/Girlshorts/${e.id}/girlsshorts`} id="card-1">
              <div id="img-1">
