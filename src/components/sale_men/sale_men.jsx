...
export const Salemen = () => {
  const [salemen, setsalem] = useState([]);

  useEffect(() => {
    async function getData() {
      const data = await fetch("https://ancient-dawn-26797.herokuapp.com/sale_men?_limit=12").then((d) =>
        d.json()
      );
      setsalem(data);
    }
  ...
  return (
    <div className="for">
      ...
      <div id="div-6">
        {salemen.map((e) => {
          return (
            <Link to={`/Salemen/${e.id}/sale_men`} id="card-1">
              <div id="img-1">
...
// {salemen.map(e =>
// <Link to={`/Salemen/${e.id}/forkids`} className="mybox">
