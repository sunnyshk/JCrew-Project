...
export const Boysshorts = () => {
  const [saleboy, setsale] = useState([]);

  useEffect(() => {
    async function getData() {
      const data = await fetch("https://ancient-dawn-26797.herokuapp.com/boysshorts?_limit=12").then((d) =>
        d.json()
      );
      setsale(data);
    }
  ...
  return (
    <div className="for">
      ...
      <div id="div-6">
        {saleboy.map((e) => {
          return (
            <Link to={`/Boysshorts/${e.id}/boysshorts`} id="card-1">
              <div id="img-1">
