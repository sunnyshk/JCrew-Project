...
export const Forgirls = () => {
  const [forgirls, setforgirls] = useState([]);

  useEffect(() => {
    async function getData() {
      const data = await fetch("https://ancient-dawn-26797.herokuapp.com/forgirls?_limit=12").then((d) =>
        d.json()
      );
      setforgirls(data);
    }
  ...
  return (
    <div className="for">
      ...
      <div id="div-6">
        {forgirls.map((e) => {
          return (
            <Link to={`/Forgirls/${e.id}/forgirls`} id="card-1">
              <div id="img-1">
...
// {forgirls.map(e =>
// <Link to={`/Forgirls/${e.id}/forgirls`} className="mybox">
