...
export const Boyshort = () => {
  const [saleboy, setsale] = useState([]);

  useEffect(() => {
    async function getData() {
      const data = await fetch("https://ancient-dawn-26797.herokuapp.com/boyshorts?_limit=12").then((d) =>
        d.json()
      );
      setsale(data);
    }
  ...
  return (
    <div className="for">
      ...
      <div id="top2">
        <h1 id="top-text2">Boy's Shorts</h1>
      </div>
      <div id="top3">
        <select name="" id="">
      ...
      <div id="div-6">
        {saleboy.map((e) => {
          return (
            <Link to={`/Boyshirts/${e.id}/boyshorts`} id="card-1">
              <div id="img-1">
