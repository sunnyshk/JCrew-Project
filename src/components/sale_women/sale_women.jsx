...
export const Salewomen = () => {
  const [salew, setsale] = useState([]);

  useEffect(() => {
    async function getData() {
      const data = await fetch("https://ancient-dawn-26797.herokuapp.com/sale_women?_limit=12").then((d) =>
        d.json()
      );
      setsale(data);
    }
  ...
  return (
    <div className="for">
      ...
      <div id="div-6">
        {salew.map((e) => {
          return (
            <Link to={`/Salewomen/${e.id}/sale_women`} id="card-1">
              <div id="img-1">
...
// {salew.map(e =>
// <Link to={`/Salewomen/${e.id}/sale_women`} className="mybox">
